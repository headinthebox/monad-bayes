...
instance (Ord a, Floating a) => Num (LogDomain a) where
  x + y = if y > x then fromLog (toLog y + log (1 + exp (toLog x - toLog y)))
                   else if x > y then fromLog (toLog x + log (1 + exp (toLog y - toLog x)))
                                 else toLogDomain (fromLogDomain x + fromLogDomain y)
